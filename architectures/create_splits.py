...
path_output = os.path.join('..','..', 'data', 'splits')
    ...
    if os.path.splitext(file)[0].split('_')[-1] == 'mini':
        ...
        with open(path_pointer) as f:
            for l in f.readlines():
                ...
                s4 = str(WRONG_CATEGORIES[int(s2)][2])
                pointer.append([s1, s2, s3, s4])
        pointer = np.asarray(pointer)
        np.savetxt(os.path.join(path_output, file), pointer, fmt='%s')


correct_tmp = np.asarray(CATEGORIES)
labels = correct_tmp[:, 0].astype(int)
correct_sc = correct_tmp[:, 2].astype(int)
correct_mapping = np.zeros((len(np.unique(labels)), len(np.unique(correct_sc))))
for _ in range(len(labels)):
    correct_mapping[labels[_], correct_sc[_]] = 1
np.savez('correct_mapping.npz', data=correct_mapping)
np.savez('correct_sc_map.npz', data=correct_sc)

wrong_tmp = np.asarray(WRONG_CATEGORIES)
labels = wrong_tmp[:, 0].astype(int)
wrong_sc = wrong_tmp[:, 2].astype(int)
wrong_mapping = np.zeros((len(np.unique(labels)), len(np.unique(wrong_sc))))
for _ in range(len(labels)):
    wrong_mapping[labels[_], wrong_sc[_]] = 1
np.savez('wrong_mapping.npz', data=wrong_mapping)
np.savez('wrong_sc_map.npz', data=wrong_sc)
