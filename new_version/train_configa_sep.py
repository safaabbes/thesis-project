...
from models import resnet50_2h
...
def main():
    ...
    args.path_weights = os.path.join('..', '..','data', 'new_exps', 'configa_sep', args.exp)
...
def run_train(args, logger, seed):
    ...
    args.path_weights = os.path.join('..', '..','data', 'new_exps', 'configa_sep', args.exp , str(seed))
    os.makedirs(args.path_weights, exist_ok=True)
    ...
    # Get the model
    if args.model_type.lower() == 'resnet50_2h':
    ...
    plot_dist_matrix(model.head1, args)    
    plot_dist_matrix(model.head2, args)  
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if epoch % args.freq_saving == 0:
            ...
            plot_dist_matrix(model.head1, args, epoch)  
            plot_dist_matrix(model.head2, args, epoch)  
...
def do_epoch_valid(loader_valid_source, model, criterion1, args):
    ...
    for data_source in loader_valid_source:
        ...
        with torch.inference_mode():
            ...
            # Forward pass for source data
            logits1_source,_ = model(images_source)
            _, preds1_source = torch.max(logits1_source, dim=1)
...
def plot_dist_matrix(head, args, epoch=None):
    ...
    if epoch is None:
        if len(distance_mat) == args.num_categories1:
            ...
            plt.savefig(os.path.join(args.path_weights,'sc_dist_0.png'), format='png', dpi=300)
            np.savez(os.path.join(args.path_weights, 'sc_dist_0.npz'), data=distance_mat)
    else:
        if len(distance_mat) == args.num_categories1:
