...
def main():
    ...
    args.path_weights = os.path.join('..', '..','data', 'new_exps', 'config_E', args.exp)
...
def run_train(args, logger, seed):
    ...
    args.path_weights = os.path.join('..', '..','data', 'new_exps', 'config_E', args.exp , str(seed))
    os.makedirs(args.path_weights, exist_ok=True)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        # Training
        since = time.time()
        stats_train = do_epoch_train(loader_train_source, loader_train_target, model, criterion1, criterion2, optimizer, args, logger)
...
def do_epoch_train(loader_train_source, loader_train_target, model, criterion1, criterion2, optimizer, args, logger):

    # Set the model in training mode
    model = model.train()

    # Init stats
    running_loss, running_source_loss1, running_source_loss2, running_target_loss2 = list(), list(), list(), list()
    ...
    for i, data_source in enumerate(loader_train_source):
        ...
        images_target, _, categories2_target = data_target
        ...
        #         # else:
        #         #     logger.info('Cluster {} Removed (contains 1 element)'.format(cluster))
        ...
        # source_loss2 = torch.sum(cumulative_logits_entropy_loss_tensor, axis=0) / len(cumulative_logits_entropy_loss_tensor)
        ...
        for logit in logits1_target:
            ...
            cumulative_logits_entropy_loss.append(logit_entropy_loss)
        cumulative_logits_entropy_loss_tensor = torch.stack([cumulative_logits_entropy_loss[i] for i in range(len(cumulative_logits_entropy_loss))])
        target_loss2 = torch.sum(cumulative_logits_entropy_loss_tensor, axis=0) / len(cumulative_logits_entropy_loss_tensor)
        ...
        target_loss2 = args.mu3 * target_loss2
        loss = source_loss1 + source_loss2 + target_loss2
        ...
        running_target_loss2.append(target_loss2.item())
    ...
    stats = {
        ...
        'target_loss2': np.mean(running_target_loss2),
...
def update_wandb(epoch, stats_train, stats_valid):
    ...
    wandb.log({
        ...
        "train/target_loss2": stats_train['target_loss2'].item(),
