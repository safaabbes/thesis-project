...
def main():
    ...
    # Create logger
    path_log = os.path.join(args.path_weights, 'pca_{:s}_{:s}.txt'.format(args.test_domain, args.checkpoint))
...
def run_test(args, logger, checkpoint):
    ...
    if args.model_type.lower() == 'resnet50s':
        np.savez(os.path.join(args.path_weights,'F_2H_{}.npz'.format(args.test_domain)), 
        ...
        np.savez(os.path.join(args.path_weights,'F_1H_{}.npz'.format(args.test_domain)), 
